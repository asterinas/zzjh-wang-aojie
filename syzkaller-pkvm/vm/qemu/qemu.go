...
func (inst *instance) boot() error {
	...
	inst.qemu = qemu
	// Qemu has started.
	log.Logf(0, "vm %v ssh port: %v", inst.index, inst.SSHOptions.Port)
	// Start output merger.
	var tee io.Writer
	if inst.debug {
		tee = os.Stdout
...
func (inst *instance) Run(ctx context.Context, command string) ( // cmd := fmt.Sprintf("%v runner %v %v %v", executorBin, inst.Index(), host, port)
	...
	if bin := filepath.Base(args[0]); inst.target.HostFuzzer && bin == "syz-execprog" {
		...
		args = append(args, inst.User+"@localhost", "cd "+inst.targetDir()+" && "+command) // running in VMs
	}
	if inst.debug {
